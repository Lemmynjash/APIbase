...
> One MCP endpoint. 476 tools. 145 providers. Pay per call with x402 (USDC on Base) or MPP (USDC on Tempo).
...
## Tool Categories (476 tools, 145 providers)
...
GET  /api/v1/tools                      → Live tool catalog (all 476 tools, JSON schemas)
...
**Progressive disclosure:** Instead of loading all 476 tool schemas into context, agents can call the `discover_tools` prompt to find relevant tools first:
