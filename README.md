...
> One MCP endpoint. 460 tools. 140 providers. Pay per call with x402 (USDC on Base) or MPP (USDC on Tempo).
...
Production MCP server that gives AI agents access to 460 real-world API tools through a single endpoint. Agents connect once to `https://apibase.pro/mcp` and can search flights, get stock quotes, translate text, check weather alerts, generate images, send emails, look up holidays, shorten URLs, detect fires by satellite, decode VINs, look up chemical compounds, find EV chargers, batch multiple calls, track usage analytics — and 250+ more tools across 30+ categories.
...
## Tool Categories (460 tools, 140 providers)
...
GET  /api/v1/tools                      → Live tool catalog (all 460 tools, JSON schemas)
...
**Progressive disclosure:** Instead of loading all 460 tool schemas into context, agents can call the `discover_tools` prompt to find relevant tools first:
